...
const __section_4_1 = ({ data }: any) => {
    const termsconditionsdata = data && ULtoArray(data);

    return (
        <>
            <div className='w-full text-[#3944B3] text-[18px] font-normal mt-[27px] mx-6 dark:text-[#fff]'>Terms & Conditions</div>
            {
                termsconditionsdata &&
                termsconditionsdata.length > 0 ?
                termsconditionsdata?.map((item: any, idx: number) => (
                    <div className={`mt-[17px] ${termsconditionsdata.length - 1 == idx && "mb-[30px]"} px-[30px] flex`}>
